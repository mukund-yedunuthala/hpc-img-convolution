...
The program has been implemented on the following two images:
\begin{itemize}
    \item 512 x 512 pixel grayscale image.
    \item 8162 x 8162 pixel grayscale image.
\end{itemize}
No suited external libraries have been used or implemented in the code belonging to this project.
...
are written to file as per the specifications, which can then be converted to different formats using ImageMagick application. 
...
The aim of this kernel is to smoothen an image, or reduce the noise or detail within an image. The 3 x 3 matrix for this operation is 
specified below. 
$$
    \frac{1}{16}
    \begin{bmatrix}
...
$$
\subsection{Sharpen}
The aim of this kernel is to enhance the sharpness of the image, can be understood to be the opposite of blur. The 3 x 3 matrix for this operation
is specified below. 
$$
    \begin{bmatrix}
        0 & -1 & 0 \\ -1 & 5 & -1 \\ 0 & -1 & 0
...
points where the brightness suddenly changes, or has discontinuities. The 3 x 3 matrix used for this operation is shown below. 
$$
\begin{bmatrix}
    -1 & -1 & -1 \\ -1 & 8 & -1 \\ -1 & -1 & -1
\end{bmatrix}
...
in order to finish the computation of new pixel value. This is achieved by using modulo operator within the loops, as opposed
to another popular method known as padding. 
...
The figure representing this division can be seen here. Subsequent execution of this program is achieved using university cluster's PBS queue system, using a script file.
The wall times are then tabulated. 
\begin{figure}[h!]
    \centering
    \includegraphics[scale=0.5]{"MPI".png}
    \caption{Distribution of data between processes}
    \label{Fig.1}
\end{figure}
\newpage
\section{Results}
At the beginning, the results belonging to the 512 x 512 image are presented.
\vfill
\begin{figure}
    \centering
    \begin{subfigure}{.45\textwidth}
        \centering
        \includegraphics[scale=0.4]{./"512".png}
        \caption{The original image}
        \label{Fig.2a}
    \end{subfigure}
    \begin{subfigure}{.45\textwidth}
        \centering
        \includegraphics[scale=0.4]{"result01-edgedetection-512".png}
        \caption{After application of edge detection}
        \label{Fig.2b}
    \end{subfigure}
    \begin{subfigure}{.45\textwidth}
        \centering
        \includegraphics[scale=0.4]{"result02-blur-512".png}
        \caption{After application of gaussian blur}
        \label{Fig.2c}
    \end{subfigure}
    \begin{subfigure}{.45\textwidth}
        \centering
        \includegraphics[scale=0.4]{"result03-all-512".png}
